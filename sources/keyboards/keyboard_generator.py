...
class KeyboardGenerator:
    ...
    def watch_day_keyboard(
        self,
        data_factories: List[MatchDayCallbackFactory],
    ...
    def admin_watch_day_keyboard(
        self,
        data_factories: List[AdminMatchDayCallbackFactory],
        buttons_info: list[NearestMeetingsSchema],
    ...
    def admin_create_watch_day_keyboard(
        self,
        data_factories: List[AdminCreateWatchDayCallbackFactory],
    ...
    def places_editor_keyboard(
        self,
        data_factories: List[PlacesEditorFactory | WatchPlaceChangeFactory],
        buttons_info: List[PlacesSchema],
    ):
        inline_keyboard = [
            [self.__place_button(factory_data, buttons_info)]
            for factory_data in data_factories
        ]
        back_to_main_menu = InlineKeyboardButton(
            text="Назад в меню", callback_data="back_to_main_menu"
    ...
    def __button(
        ...
        try:
            btn_data = list(
                filter(lambda i: callback_data.id == i.match_day_id, button_data)
            )
    ...
    def __button_2(
        ...
        try:
            btn_data = list(
                filter(lambda i: callback_data.id == i.event_id, button_data)
            )
    ...
    @staticmethod
    def __place_button(
        callback_data: PlacesEditorFactory, button_data: list[PlacesSchema]
    ) -> InlineKeyboardButton:
        try:
            btn_data = list(filter(lambda i: callback_data.id == i.id, button_data))
            button_name = f"{btn_data[0].place_name} -- {btn_data[0].address}"
